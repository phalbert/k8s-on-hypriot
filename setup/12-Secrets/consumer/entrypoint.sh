...
if [[ $1 = "confd" ]]; then
    ...
    if [[ -z "${VAULT_ROLE_ID}"   ]]; then echo -e "[ ERROR ] Vault RoleID must be provided explicitly. Exit.";  exit 1; fi
    if [[ -z "${VAULT_SECRET_ID}" ]]; then echo -e "[ ERROR ] Vault SecretID must be provided explicitly. Exit.";  exit 1; fi
    ...
    if [[ $(curl --silent ${VAULT_SCHEME}://${VAULT_HOST_ADDR}:${VAULT_TCP_PORT}/v1/sys/seal-status | jq .sealed) == "true" ]]
    then
        echo -e "[ ERROR ] Vault is sealed"
        # TODO: here a message to the respective Slack channel may be sent
        exit 1
    fi
    ...
    if [[ -e /tmpfs/annotations ]]; then sed -e '/kubernetes.io/d' /tmpfs/annotations | grep "${ANNOTATION_PREFIX}" | sed -e "s/${ANNOTATION_PREFIX}//" > /secrets.lst; fi
